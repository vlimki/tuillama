...
fn conversation_context_token_count(app: &App) -> usize {
    let mut chars = app
        .system_prompt
        .as_deref()
        .unwrap_or_default()
    ...
    chars += app
        .messages
        .iter()
        .map(|m| m.content.chars().count() + m.thinking.as_deref().unwrap_or_default().chars().count())
        .sum::<usize>();
    chars += app.pending_assistant.chars().count();
    chars += app.pending_thinking.chars().count();
    chars += app.input.chars().count();
    ((chars as f32) / 4.0).ceil() as usize
}

fn configured_context_window(app: &App) -> Option<usize> {
...
fn compact_context_window(tokens: usize) -> String {
...
fn context_budget_label(app: &App) -> String {
    let used = conversation_context_token_count(app);
    match configured_context_window(app) {
        Some(limit) => format!("{} / {}", compact_token_count(used), compact_context_window(limit)),
        None => format!("{} / unknown", compact_token_count(used)),
    }
}
    ...
    match &app.popup {
        ...
        Popup::Help => {
            ...
            let lines: Vec<Line> = vec![
                ...
                Line::from("  : command mode (:attach PATH, :web on, :set key value)"),
                ...
                Line::from("  :set model qwen3.6:27b or :set options.num_ctx 32768"),
...
fn draw_stats_panel(frame: &mut ratatui::Frame, area: Rect, app: &App) {
    ...
    let metrics = vec![
        ...
        Line::from(vec![
            ...
            Span::styled(context_budget_label(app), Style::default().fg(app.theme.stats_value).add_modifier(Modifier::BOLD)),
        ]),
        Line::from(vec![
